...
# -------- LLM provider adapter: DashScope/Qwen -> OpenAI-like dict --------
def _as_openai_like_choices(output_choices):
    # 标准化为 [{"message": {"content": "..."}}, ...]
    norm = []
    for c in output_choices:
        # 兼容对象或字典两种返回
        msg = getattr(c, "message", None) or c.get("message", {})
        content = getattr(msg, "content", None) or msg.get("content", "")
        norm.append({"message": {"content": content}})
    return norm

def llm_generate_with_dashscope(model, messages, temperature=0.7, n=1, top_p=None, stop=None, extra=None):
    """
    调用阿里百炼（DashScope）对话补全，返回 OpenAI ChatCompletion 风格的 dict:
    {
    ...
    params = dict(
        model=model,
        messages=messages,
        result_format="message",        # 要 message 对象
    ...
    # 兼容属性/字典两种风格取值
    status = getattr(resp, "status_code", None) or resp.get("status_code")
    if status == HTTPStatus.OK:
        output = getattr(resp, "output", None) or resp.get("output", {})
        choices = getattr(output, "choices", None) or output.get("choices", [])
        usage = getattr(resp, "usage", None) or resp.get("usage", {})  # 包含 input_tokens/output_tokens/total_tokens

        return {
            "choices": _as_openai_like_choices(choices),
            "usage": {
        ...
        # 取更友好的错误信息
        code = getattr(resp, "code", None) or resp.get("code")
        ...
        while True:
            ...
            for attempt in range(1000):
                try:
                    ...
                    dash_model = model_map.get(model, model)  # 允许你直接填 qwen-plus / qwen-turbo / qwen-max / qwen2.5-72b-instruct 等

                    response_cur = llm_generate_with_dashscope(
                        model=dash_model,
                        messages=messages,
                        temperature=cfg.temperature,
                        n=chunk_size,
